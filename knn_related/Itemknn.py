...
def topKMatches(prefer,AVG, person, users, items, limit_num = 10, sim=MyPearson):
	scores = []
	...
	for other in items:
		...
		if simi > 0.01 and num >= limit_num:
...
def loadMovieLensTrain(fileName='u1.base'):
	...
	for line in open(fileName,'r'):       # 打开指定文件
		...
		prefer.setdefault(int(movieid)-1, {})      # 设置字典的默认格式,元素是user:{}字典
		prefer[int(movieid)-1][int(userid)-1] = float(rating)    
		AVG.setdefault(int(movieid)-1,0.0)
		AVG[int(movieid)-1] += float(rating) 
		num.setdefault(int(movieid)-1,0.0)
		num[int(movieid)-1] += 1.0  
	for movieid in AVG:
		AVG[movieid] /= num[movieid]
...
def getAllUserRating(fileTrain='u1.base', output='u1.test',itemsfile="../movie/movies_new.csv"):
	...
	time1 = datetime.now()
	for i,movie in enumerate(items):  # test集中每个项目
		if i%500==0:
			print "id:",movie
		if movie not in traindata:
			continue
		limit_num = 25
		rating = topKMatches(traindata, AVG, movie, users, items, limit_num)  # 基于训练集预测用户评分(用户数目<=K)
		length = len(rating)
		while length < 100 and limit_num > 20:
			limit_num -= 1
			rating = topKMatches(traindata, AVG, movie, users, items, limit_num)
			length = len(rating)
		f.write(str(movie))
		if rating!=[]:
			for x in rating:
...
if __name__ == "__main__":
	print "Usage python Itemknn.py traindata output test_data"
