...
def predict(user, item, traindata, dic_sim):
	ws = 0.00
	total = 0.00
	for user2 in dic_sim[user]:
		if user2 in traindata and item in traindata[user2]:
			ws += dic_sim[user][user2]
			total += dic_sim[user][user2] * traindata[user2][item]
	if ws > 0:
		return total/ws, ws
	else:
		return 0.00, 0.00
...
def loadMovieLensTrain(fileName='u1.base'):
	...
	for line in open(fileName,'r'):
		...
		prefer.setdefault(int(userid)-1, {})
...
def main_deal(sim_file, train_file, test_file, num_K, output):
	...
	for line in f:
		...
		user = int(line[0]) - 1
		item = int(line[1]) - 1
		if user not in traindata or user not in dic_sim:
